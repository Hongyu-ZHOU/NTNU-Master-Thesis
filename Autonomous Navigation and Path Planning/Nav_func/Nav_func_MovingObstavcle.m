function  [ObstacleIdx] = Nav_func_MovingObstavcle(ObstacleIdx,X,Y,r_world,r_o,k,n,Mode)
% Nav_func_MovingObstavcle calculates the positions of moving obstacles
% 
% Hongyu Zhou
% 
...
switch Mode
    ...
    case 5 % Three obstacles, head on
        ...
        if k >= 1 && k <= 9
            if norm(pos_2)+r_o <= r_world
                Obstacle_2(2) = Obstacle_2(2)+n;
            end
        end
        
        if k >= 1 && k <= 9
            if norm(pos_3)+r_o <= r_world
